...
def vlimassegiempat (sisi, tinggi) :
    V = round (1/3 * sisi ** 2 * tinggi)
    return V

#limas segitiga
def vlimastiga (alas, t_alas, t_limas) :
    L = round (0.5 * alas * t_alas + 3 * (0.5 * alas * t_limas))
    return  L

def vlimassegitiga (alas, t_alas, t_limas) :
    V = round (1/3 * 0.5 * alas * t_alas * t_limas)
    return V

#Prisma Segitiga
def llimassegitiga (alas, t_alas, t_prisma) :
    L = round (2 * (0.5 * alas * t_alas + alas * t_prisma + 0.5 * alas * t_alas))
    return L

def vlimassegitiga (alas, t_alas, t_prisma) :
